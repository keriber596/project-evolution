	...
	void generate(int x, int y, int** heights, bool**& isSolid, bool** isUsed, int seed) {
		if (type == BTypes::Divide) {
			...
			for (auto& p : points) {
				if (!isUsed[p.x][p.y]) {
					if (coef.generate() % 2 == 0) {
						heights[p.x][p.y] += 3 * randShift.generate();
					}
					else {
						heights[p.x][p.y] -= 3 * randShift.generate();
					}
...
IntPair defineCloseVecForMap1(std::vector<IntPair> vec, int x, int y, int** map, int radius) {
	IntPair min = vec[0];
	IntPair max = vec[0];
	...
	if (c < 0.3 * vec.size()) {
		return IntPair(index, 3);
	}
	else if (c < 0.5 * vec.size()) {
...
IntPair defineCloseVecForMap2(std::vector<IntPair> vec, int x, int y, int** map, int radius) {
	IntPair min = vec[0];
	IntPair max = vec[0];
...
std::vector<LitosBorder> defineLitosBorders(LitosPlate* litos, int size, int x, int y, int** expMap, int** leavMap, int maxShift, int minShift, int seed) {
	...
	for (int i = 1; i < size; i++) {
		tmp = defineCloseVecForMap1(litos[i].expansion, x, y, leavMap, 30);
		if (tmp.x == -1) {
			...
			std::vector<IntPair> vec = mergeVectors(litos[i].leaving, litos[tmp.x].expansion);
			borders.push_back(LitosBorder(randH.generate(), 10, 5, vec, BTypes::Highland));
			notForUse[tmp.x] = true;
		}
		else if (tmp.y == 1) {
			std::vector<IntPair> vec = mergeVectors(litos[i].leaving, litos[tmp.x].expansion);
			borders.push_back(LitosBorder(randH.generate(), 10, 5, vec, BTypes::Plain));
			notForUse[tmp.x] = true;
		}
	}
	...
	for (int i = 1; i < size; i++) {
		if (!notForUse[i]) {
			tmp = defineCloseVecForMap2(litos[i].leaving, x, y, expMap, 30);
			if (tmp.x == -1) {
